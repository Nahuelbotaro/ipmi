...
// teclas + y - controlan el tamaño extra de los cuadrados
void keyPressed() {
  if (key == 'q') {
  ...
  // tecla R también activa y desactiva la rotación
  if (key == 'r' || key == 'R') {
    rotando = !rotando;
  }

  // barra espaciadora reinicia todo
  if (key == ' ') {
    resetearValores();
  }
}
